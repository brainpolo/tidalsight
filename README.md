...
- **[AI report card](REPORT_CARD.md)** — Holistic buy/sell/hold assessment across up to 6 dimensions (Finance, Sentiment, Risk, Valuation, Product, People) each scored 1–4, synthesised into an overall verdict with a 12-month price target. Built on Herzberg's Two-Factor Theory separating hygiene factors (1.5x weight) from motivators (1.0x). Equities use all 6; crypto/commodities/currencies use 4 (scores scaled to the same 0–30 range). Motivator scores are personalised by user notes via dedicated revision agents, with a Personal/Base toggle to compare.
